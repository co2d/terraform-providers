...
function linkify(src) {
  if (!src) return "—";
  const url = src;
  const pretty = src.replace(/^https?:\/\//, "").replace(/\/$/, "");
  return `<a href="${url}" target="_blank" rel="noopener">${pretty}</a>`;
}
function colorAccent(idx = 0) {
  const p = ["#a737ff", "#911ced", "#0c56e9", "#0046d1", "#1060ff"];
  return p[idx % p.length];
}
async function loadJSON(p) {
  const res = await fetch(p, { cache: "no-store" });
  if (!res.ok) throw new Error(`Failed ${p}: ${res.status}`);
  return res.json();
}
function pickWindowData(key) {
  const w = METRICS.windows[key];
  return w && w.available ? w : null;
}
...
function buildTopChart(rows) {
  ...
  charts.top.setOption({
    ...
    series: [{
      ...
      barMaxWidth: 16, showBackground: true, backgroundStyle: { color: "#f1f2f3" }
    }]
  });
}
function buildMoversChart(moversUp, moversDown) {
  const up = moversUp.slice(0, 10);
  const down = moversDown.slice(0, 10);
  const names = [...up.map(x => x.fullName), ...down.map(x => x.fullName)];
  const deltas = [...up.map(x => x.change), ...down.map(x => -x.change)];
  const colors = names.map((_,i) => i < up.length ? "#008a22" : "#e52228");
  charts.movers.setOption({
    backgroundColor: "transparent",
    tooltip: { trigger: "axis", axisPointer: { type: "shadow" },
      formatter: (p) => {
        const a = p[0]; const dir = a.value >= 0 ? "▲" : "▼";
        return `${a.name}<br/>Rank change: <b>${dir} ${Math.abs(a.value)}</b>`;
      }},
    grid: { left: 8, right: 16, bottom: 10, top: 20, containLabel: true },
...
function rebuildTable(windowKey) {
  ...
  const rankMapNow = new Map();
  PROVIDERS.providers.slice().sort((a,b)=> b.totalDownloads - a.totalDownloads)
    .forEach((p,i)=> rankMapNow.set(p.fullName, i+1));

  let rows = PROVIDERS.providers.map((p,i) => {
    const periodDownloads = w ? (w.top.find(x => x.fullName === p.fullName)?.periodDownloads ?? 0) : 0;
    const before = w && w.moversUp.concat(w.moversDown).find(x => x.fullName === p.fullName)?.before;
    const now = rankMapNow.get(p.fullName);
    const delta = (before && now) ? (before - now) : 0;
    return { idx: i+1, ...p, periodDownloads, delta };
  });

  if (q) rows = rows.filter(r => r.fullName.toLowerCase().includes(q));

  document.querySelector("#catalogCountTag").textContent = `${rows.length} shown`;
  document.querySelector("#thPeriod").textContent = w ? `Period downloads (${windowKey.replace("last_","")})` : "Period downloads";
...
async function boot() {
  ...
  document.querySelector("#searchBox").addEventListener("input", e => rebuildTable(CURRENT_WINDOW));
  syncWindowUI();
}
function syncWindowUI() {
  const w = pickWindowData(CURRENT_WINDOW);
  const ref = w && w.refDate ? `ref: ${w.refDate}` : "ref: N/A";
  document.querySelector("#refDateTag").textContent = ref;
  if (w) {
    buildTopChart(w.top);
    buildMoversChart(w.moversUp, w.moversDown);
    document.querySelector("#topCountTag").textContent = `${w.top.length} items`;
  } else {
    charts.top.clear(); charts.movers.clear();
    document.querySelector("#topCountTag").textContent = `0 items`;
  }
